 ...
 * It is as independent from the OWL API as possible. However, it is still
 * necessary to test for the reachability of an ontology and moreover to resolve
 * its URI for that of its source. For these reasons we still need a parser of
 * OWL files here.
 */
public class AServProtocolManager implements Service {
     ...
     * Creation
     *
     * @param dir: a hash-table of all directories by which the service will be
     * registred
     */
    ...
    public Message load(Properties params) {
        ...
        if (pretty == null) {
            pretty = params.getProperty("pretty");
        }
    ...
    public Message align(Properties params) {
        ...
        for (Entry<Object, Object> e : commandLineParams.entrySet()) {
            if (params.getProperty((String) e.getKey()) == null) {
                params.setProperty((String) e.getKey(), (String) e.getValue());
            }
        }
    ...
    /**
     * returns null if alignment not retrieved Otherwise returns AlignmentId or
     * an ErrorMsg
     *
    ...
    public Message getAlignments(Properties params) {
        ...
        try {
            if (uri != null && !uri.equals("")) {
                alignments = alignmentCache.getAlignmentByURI(uri);
            }
    ...
    /**
     * Returns only the metadata of an alignment and returns it in parameters
     *
     * @param params: the parameters of the query
     * @return a message containing the query result
     */
     ...
     *
     * All these primitives must create a new alignment and return its Id There
     * is no way an alignment server could modify an alignment
     ********************************************************************
     */
    /**
     * Trim an alignment
    ...
    public Message eval(Properties params) {
        ...
        if (classname == null) {
            classname = "fr.inrialpes.exmo.align.impl.eval.PRecEvaluator";
        }
        Evaluator eval = null;
        try {
    ...
    public Message matchOntologyNetwork(Properties params) {
        ...
        if (params.getProperty("reflexive") != null) {
            reflexive = true;
        }
        if (params.getProperty("symmetric") != null) {
            symmetric = true;
        }
        String id = params.getProperty("id");
        String method = params.getProperty("method");
    ...
    public Message closeOntologyNetwork(Properties params) {
        ...
        if (params.getProperty("transitive") != null) {
            trans = true;
        }
        if (params.getProperty("reflexive") != null) {
            refl = true;
        ...
        if (params.getProperty("new") != null) {
            newnet = true;
        }
        if (newnet) {
            String pretty = params.getProperty("pretty");
    ...
    public Message setopOntologyNetwork(Properties params) {
        ...
        try {
            if (op.equals("meet")) {
                noo = BasicOntologyNetwork.meet(noo1, noo2);
            } else if (op.equals("join")) {
                noo = BasicOntologyNetwork.join(noo1, noo2);
            } else if (op.equals("diff")) {
                noo = BasicOntologyNetwork.diff(noo1, noo2);
            }
        } catch (AlignmentException e) {
    ...
    protected String registerNetwork(BasicOntologyNetwork noo, String id) {
        String newid = id;
        if (id == null) {
            newid = alignmentCache.recordNewNetwork(noo, true);
        }
     ...
     *
     * - publication (to some directory) registerID publishServices unregisterID
     * (publishRenderer) (publishMethods) : can be retrieved through the
     * classical interface. requires a direcory
     *
     * - subscribe style subscribe() : ask to receive new metadata notify(
     * metadata ) : send new metadata to subscriber unsubscribe() : update(
     * metadata ) : update some modification requires to store the subscribers
     *
     * - query style: this is the classical protocol that can be done through
     * WSDL getMetadata() getAlignment() requires to store the node that can be
     */
    /**
     *
     * @param params: the parameters of the query
     ...
     * (when it was in static) Remedied (for the moment by improving the visited
     * cache) This may also benefit by first filling the visited by the path of
     * the libraries we know.
     ********************************************************************
     */
    /**
     * Display all the classes inheriting or implementing a given interface in
     * the currently loaded packages.
     *
     * @param toclass: the interface to implement
     * @return the set of strings corresponding to the implementation of the
     * class
     */
    ...
    public boolean implementsInterface(String classname, Class<?> tosubclass) {
        try {
            ...
            if (!Modifier.isAbstract(mod)
                    ...
                    && tosubclass.isAssignableFrom(cl)) {
                return true;
            }
